...
describe("skillInvocation", () => {
	...
	describe("resolveSkillContentForMode", () => {
		it("returns null when skillsManager is undefined", async () => {
			const result = await resolveSkillContentForMode(undefined, "test-skill", "draft")
		...
		it("delegates to skillsManager.getSkillContent with correct arguments", async () => {
			...
			const result = await resolveSkillContentForMode(skillsManager, "test-skill", "outline")
			expect(skillsManager.getSkillContent).toHaveBeenCalledWith("test-skill", "outline")
		...
		it("returns null when skillsManager returns null", async () => {
			...
			const result = await resolveSkillContentForMode(skillsManager, "nonexistent", "draft")
			expect(result).toBeNull()
		})
	})
